...
// It's safe to initialize Genkit here because this file is only loaded at runtime on the server.
const ai = genkit({
    plugins: [googleAI()],
    logLevel: 'warn',
...
export async function handleScrape(req: Request) {
    try {
        const body = await req.json();
        const { productUrl, categoryNames } = ScrapeProductInputSchema.parse(body);

        const result = await scrapeProductFlow({ productUrl, categoryNames });
